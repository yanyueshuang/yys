...
<script language="javascript">
	...
	function exportExcel() {
		...
		//获取考生value值
		var name = $("#name").val()
		var  sect=document.getElementById("sel");
		//alert("sect:"+sect);
		...
		//跳转页面
		location.href = encodeURI("<%=request.getContextPath()%>/manage/classs/importExaminee?classId="+classs_id+"&name="+name+"")
						...
						<td align="center" valign="top">
							<form
								...
								<table width="96%" height="40" border="0" cellpadding="0"
									cellspacing="0">
									<tr>
										<td align="left" valign="middle">&nbsp; 所在班级： <select
											name="classId" id="sel">

										</select> &nbsp;考生姓名： <input type="text" name="name"
											value="<c:if test="${name!=null}">${name}</c:if>" id="name" />&nbsp;&nbsp;
											<input type="submit" class="btn_grey" value=" 查询 " />
										</td>
										<td><input type="button" value="导出学生的信息"
											onclick="exportExcel()" /></td>
									</tr>
									<tr>
										<td>导入学生的信息：<input id="fed" type="file" name="fe" /></td>
										<%-- 									<td><a href="<%=request.getContextPath()%>/exam/考生信息t填写模板.xls">下载excel模板</a></td> --%>
										<td><input type="button" value="导入学生的信息"
											onclick="importExcel()" /></td>
									</tr>
									</tr>
								</table>
							</form>
									...
									<c:forEach items="${zhongList}" var="map" varStatus="s">
										<tr>
											...
											<td><a class="btn btn-default btn-sm"
												href="${pageContext.request.contextPath}/interviewee/proposer?visitorId=${sessionScope.visitor.get("
